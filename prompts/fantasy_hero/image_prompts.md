...
```
Black and white line art coloring page, pure black outlines on white background only, absolutely no color, no shading, no gradients, no fills, pen and ink style. A {age}-year-old {gender} in a heroic pose with one arm outstretched, wearing {signature_look}. Full body visible from head to toe — fully visible at top and bottom, never cropped. Character centered with generous white space margin. Simple fantasy background as outlines only: enchanted trees, a castle tower, stars, a small friendly dragon. Thick clean lines suitable for children to color with crayons. Coloring book page style, printable.
```
